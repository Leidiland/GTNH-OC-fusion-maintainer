"""Generate recipes.lua from a GT5-Unofficial source checkout.

Usage: python tools/generate-recipes.py <GT5-Unofficial path> [--output recipes.lua] [--source-version 5.09.54.133]
"""
...
def extract_recipes(sources, numbers, fluids):
    ...
    for path, text in sorted(sources.files.items()):
        for match in re.finditer(r"\.addTo\(\s*fusionRecipes\s*\)", text):
            ...
            location = "{}:{}".format(path.relative_to(sources.root).as_posix(), text.count("\n", 0, start) + 1)
            try:
...
def write_lua(recipes, output, source_version):
    recipes.sort(key=lambda recipe: (recipe["output"][1], recipe["startupEu"], [fluid[0] for fluid in recipe["inputs"]]))
    lines = ["-- Generated by tools/generate-recipes.py from GT5-Unofficial " + source_version, "return {"]
...
def main():
    parser = argparse.ArgumentParser(description="Generate the fusion recipe table from GT5-Unofficial sources")
    parser.add_argument("source", type=pathlib.Path, help="Path to a GT5-Unofficial checkout")
    parser.add_argument("--output", type=pathlib.Path, default=pathlib.Path(__file__).resolve().parent.parent / "recipes.lua")
    parser.add_argument("--source-version", help="Version label written to the file header")
    arguments = parser.parse_args()

    sources = Sources(arguments.source)
    ...
    write_lua(recipes, arguments.output, arguments.source_version or detect_version(arguments.source))
